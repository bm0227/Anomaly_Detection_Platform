                        ...
                        "score": [
                                    {
                                        ...
                                        "Recall Score":round(rec_score,3),
...
@app.get('/Greenlight/Intro')
async def get_introFile():
    result_filename = "./Greenlight_Team.pdf"
    return FileResponse(result_filename, media_type='application/pdf', filename="GreenLight_Team.pdf")
...
@app.post('/onefile/')
async def get_one_result(threshold:str = Form(...), className: str = Form(...), modelName:str = Form(...), fileName:str = Form(...),  file: UploadFile = File(...)):
    """
...
# @app.post('/somefile/result_File')
# async def get_some_result_Image(result_filename : str = Form(...)):
#     print(result_filename)
#     return FileResponse("./response_result/2021-10-03_23_16_11/result.zip", media_type='application/x-zip-compressed', filename="result.zip")

# 추후 난수발생기? 문자를 암호화하는 생성기를 통해 함부로 다운을 못받게 만들자.
@app.get('/result_File/{filename}')
async def get_some_result_Image(filename : str):
    result_filename = "./response_result/"+ filename +"/result.zip"
    return FileResponse(result_filename, media_type='application/x-zip-compressed', filename="result.zip")


@app.get('/test_File/multipleFile')
async def get_some_result_Image():
    result_filename = "./TestingFolder/"+"eaxm.zip"
    return FileResponse(result_filename, media_type='application/x-zip-compressed', filename="bottle.zip")

@app.get('/test_File/oneFile')
async def get_some_result_Image():
    result_filename = "./TestingFolder/"+"008.png"
    return FileResponse(result_filename, media_type='application/png', filename="bottle.png")

@app.post('/somefile')
async def get_some_result(fileName:str = Form(...),threshold:str = Form(...), className:str=Form(...), modelName:str=Form(...), file:UploadFile = File(...)):
    """
    ...
    with open(os.path.join(dir_file_path, fileName), "wb") as fp:
        fp.write(contents)

    suvey_zip = zipfile.ZipFile(os.path.join(dir_file_path, fileName))
    ...
    image_results = make_some_image_of_json(filename=save_dir+"/"+"result.zip", result_filename="result.zip", img_roc_auc=img_roc_auc, pre_score=pre_score, rec_score= rec_score, f_score=f_score)
    return image_results
