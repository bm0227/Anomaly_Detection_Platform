...
@app.post('/onefile')
async def get_one_result(file:UploadFile = File(...), y_labels:str = None, threshold:str = None, className:str=None, modelName:str="wide_resnet50_2"):
    """
    ...
    """
    dir_file_path = make_folder(flag=False)
    ...
    args = config()
    args.setParam(dir_file_path=dir_file_path, filename=file.filename, className=className, y_labels=y_labels, threshold=threshold, modelName=modelName)

    anomalyScore = inference_one(args)
    filename = os.path.join(dir_file_path,file.filename)
    result_filename = os.path.join(dir_file_path,"result_"+ file.filename)
